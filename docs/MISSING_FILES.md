...
|---|-------|--------|-------|-----|
| 3 | `config/hypr/render.conf` | missing | **present** | Created in house style; every `source =` target now resolves. The knobs are documented but commented out — Hyprland's render keys differ between versions, so shipping active values risks an "unknown key" warning on some builds or a silent override of a faster default. |
| 4 | `config/waybar/scripts/do-not-disturb.sh` | missing | **present** | Implemented against the module's own contract (`return-type: json`, `format-icons: ["󰂞","󰂂"]` → emits `alt`). Queries `swaync-client -D`, falls back to `dunstctl`, degrades gracefully when neither exists. Verified: emits valid JSON, `alt` flips 0↔1, tooltip newlines decode correctly. |
| 9 | `.local/bin/*` | `100644` ×9 | `100755` ×9 | `git update-index --chmod=+x` |

After these changes the only remaining missing module script in `config/waybar/scripts/` is
`gpu-intel.sh` / `task-count.sh` / `updates-flatpak.sh` (the DND one is resolved).

### Deliberately not fixed

| Group | Reason |
|-------|--------|
| 🔴 `config/swaync/scripts/` (68) | Writing these means inventing 68 scripts' behaviour. The widgets define *what they should do*, but implementing the set is a feature project, not a fix. |
| 🔴 47 empty JSON stubs | Each needs a decision: the real schema/registry content, or deletion in favour of generating on demand. Guessing at schema bodies would be worse than the current no-op validation, because it would *look* like validation. |
| 🟠 Packaging inputs (8) | `packaging/aur/*/PKGBUILD`, `nix/packages/ash-dotfiles.nix`, `Dockerfile.full` and the flatpak `.metainfo.xml`/`.desktop` carry release metadata (checksums, maintainers, versions). Regenerating them is a release-engineering task. Note the flatpak `APP_ID` drift (`dev.ash_dotfiles.AshDotfiles` vs the shipped `com.ash.dotfiles.yml`) needs a decision on which ID is canonical. |
| 🟡 Symlink sources (3) | `config/git/` and `config/starship/starship.toml` need real content. The *behaviour* fix — so `ash doctor fix` reports these as errors instead of silently skipping — is a small change worth doing separately. |
| 🟡 `config/hypr/scripts/` (3), dunst (1) | `battery-monitor.sh` exists at `scripts/notification/`; the others need implementations. |
| 🟢 Comment-only (5) | Fixing these means either writing the scripts or correcting the claims in the comments. |
