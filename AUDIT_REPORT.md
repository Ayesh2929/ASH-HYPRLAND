...
## Addendum — Phase B: missing files & path-contract fixes

**Scope of this pass:** every `~/.config/…` path referenced by a *shipped* config file was
re-resolved after comment-stripping, and each actionable miss was either created or fixed.

### Files added (referenced but absent)

| Added | Referenced by |
|---|---|
| `config/dunst/dunstrc` | `scripts/health/*`, dunst rules (`include = rules/`) |
| `config/hyprlock/hyprlock.conf` | hyprlock layouts, `hyprctl`, `ash power lock` |
| `config/kanata/kanata.kbd` | `config/hypr/autostart.conf:75` |
| `config/xdg-desktop-portal/hyprland-portals.conf` | `config/hypr/ecosystem.conf` |
| `config/rofi/menus/vpn-manager/{vpn-manager.sh,vpn-manager.rasi,style.rasi}` | `config/waybar/custom-modules/vpn-status.jsonc:13` |
| `config/hypr/hyprpaper.conf` | `config/hypr/themes/wallpaper.conf` |
| `config/hypr/scripts/screenshot.sh` | `config/hypr/env.conf:451` (`$screenshotter`) |
| `config/hypr/scripts/battery-monitor.sh` | `config/hypr/autostart.conf:105` |
| `config/dunst/scripts/open-action.sh` | 41 rule entries in `config/dunst/rules/*.conf` |
| `config/waybar/scripts/{gpu-intel,task-count,updates-flatpak}.sh` | matching `custom-modules/*.jsonc` |
| `config/swaync/scripts/{on-theme-change,on-snapshot,battery-critical,screenshot-action,toggle-screenrecord}.sh` | `config/swaync/config.json` |

### Bug fixes

1. **hyprlock / hypridle path conflict (3-way).** `ash doctor check-configs` looked for
   `${_CFG}/hypr/hyprlock.conf`, `ash power lock` for `~/.config/hypr/hyprlock.conf`, while the
   shipped layouts self-source `~/.config/hyprlock/colors.conf`. The canonical location is now
   `~/.config/hyprlock/`; both the doctor check and `ash power lock` probe both locations instead
   of hard-coding one.
2. **`ash doctor check --<topic>` was documented but not implemented.** The Hyprland configs tell
   users to run `ash doctor check --binds`, `--env`, `--cursor`, `--decorations`, `--versions`, …
   The dispatcher answered *“Unknown subcommand: 'check'”*. It now maps topics onto real check
   categories, runs the quick suite, and prints a topic list for `--help`.
3. **`scripts/health/validate.sh` was stale v3.0** — it validated `hypr/core/*`, `hypr/modules/*`,
   `hypr/rules/*`, `waybar/configs/top.jsonc` and hyprlock scripts that never existed in this
   repository. Rewritten against the actual v5 tree (checkout **and** installed copy), with
   executability, JSONC (comment/trailing-comma aware) and stub checks. It now exits 0 on a clean
   tree and caught two real defects on first run: a non-executable `swaync/scripts/_lib.sh` and
   five valid waybar JSONC files that a naive parser rejected.
4. **`scripts/health/{doctor,final-check,summary}.sh`** had the same stale-path class of bug
   (13 references) — all now point at the real config files, and the "unique feature scripts"
   checks were replaced with `ash` CLI feature probes (the standalone `hypr/scripts/theme/*`
   helpers no longer exist).

### Verification

- `scripts/health/validate.sh --repo` → **0 failures** (59 passed, 2 optional warnings).
- `validate.py` → **0 errors / 0 warnings**.
- `bash -n` over all 757 tracked shell files → **0 failures**.

### Still open (next pass)

- **63 `~/.config/swaync/scripts/*.sh`** are referenced by `config/swaync/widgets/*.json`
  (night-light, network/DNS/hotspot, gaming/MangoHud/FPS, calendar, DND, media) and are not yet in
  the tree.
- Binary assets referenced but not shipped: `assets/icons/ash-system/*.svg`,
  `assets/wallpapers/default-{dark,light,minimal}.jpg`, `sounds/*.ogg`.
- Runtime/user-scoped references (`~/.config/ash/*`, `starship.toml`, `ripgrep/config`,
  `python/startup.py`, `user-dirs.dirs`, `kanshi/config`, …) are created by the installer or by the
  colour engine at runtime, not shipped as static files.
